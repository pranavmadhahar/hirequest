
"""
router_chain.py
===============

Dispatches candidate context (resume, role, history) to the correct domain chain
(ML, Advanced_ML, Data_Science) using function-based routing.

The router_chain takes the payload from FastAPI (including role, resume_context,
and history) and invokes the appropriate domain chain defined in domain_chains.py.
"""

# load API key from .env 
from dotenv import load_dotenv, find_dotenv 

from typing import Any
from langchain_core.runnables import RunnableLambda

from backend.services.domain_chains import load_domain_chain
from backend.services.summary_chain import summary_chain
# Load environment variables required by OpenAI.
load_dotenv(find_dotenv())


def route_by_role(inputs: dict[str, Any]) -> dict:
    """
    Route interview requests to the appropriate chain.

    Args:
        inputs:
            Payload containing role, interview history,
            resume context, and execution mode.

    Returns:
        Structured output generated by the selected chain.

    Raises:
        ValueError:
            If an unsupported role is provided.
    """

    role = inputs.get("role", "")
    resume_context = inputs.get("resume_context", "")
    history = inputs.get("history", "")
    mode = inputs.get("mode", "question")  # default to question
    # Final interview evaluation path.
    if mode == "summary":
        # Route to summary chain
        return summary_chain.invoke({
            "role": role,
            "history": history
        })

    # Otherwise → route to domain chain
    if role == "ML":
        chain = load_domain_chain("ML")
    elif role == "Advanced_ML":
        chain = load_domain_chain("Advanced_ML")
    elif role == "Data_Science":
        chain = load_domain_chain("Data_Science")
    else:
        raise ValueError(f"Unsupported role: {role}")

    return chain.invoke({
        "role": role,
        "resume_context": resume_context,
        "history": history
    })

# Public router entrypoint used by API endpoints.
router_chain = RunnableLambda(route_by_role)

